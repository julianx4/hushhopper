...
ROBOT_URDF_PATH = "mini_cheetah/mini_cheetah.urdf"
...
class WalkingRobotEnv(gym.Env):
    def __init__(self, GUI = False):
        ...
        self.observation_space = spaces.Box(
            low=-50,  # Ideally, replace -np.inf with realistic lower bounds
            high=50,  # Similarly, replace np.inf with realistic upper bounds
            shape=(30,),  # Total number of values in the observation vector
            dtype=np.float32
        )
        ...
        self.robotId = p.loadURDF(ROBOT_URDF_PATH, [0, 0, 0.45], 
    ...
    def step(self, action):
        max_velocity = 3#0.873  # Velocity limit in radians per second
    ...
    def _load_robot_and_set_environment(self):
        ...
        initial_base_position = [0, 0, 0.45]
    ...
    def _get_observation(self):
        ...
        joint_states = p.getJointStates(self.robotId, range(12))
    ...
    def _compute_reward(self):
        ...
        forward_distance = -current_position[0]*100.0
        ...
        height_penalty = abs(base_height - 0.3) * 10.0
        
        joint_penalty = 0
        penalty_scaling_factor = 10.0  # Adjust this to scale the joint penalty
        specific_joints = ["torso_to_abduct_fl_j", "torso_to_abduct_fr_j", 
                        "torso_to_abduct_hl_j", "torso_to_abduct_hr_j"]
        ...
        reward = forward_distance - height_penalty - stability_penalty - joint_penalty
        return reward
